...
if(!empty($_SERVER['HTTP_X_REQUESTED_WITH']) 
	...
	// check url
	if(strpos($data['url'] , "alibaba") || strpos($data['url'] , "1688.com")){
...
function get_title($html){
       ...
       if(count($res) > 1){
       		...
       		if(count($res) > 1){
       			return str_replace(array("&nbsp;" , "&amp;") , array(" " , "`") , enc($res[0]) );	
       		}
		...
		if(count($res) > 1){
			...
			if(strlen($res) < 10){
		        ...
		        if(is_array($result)){
		        	...
		        	$res .= '<table>'.$test[1].'</table>';
		        }
		        
		        $instruction = 'p.description';
			    $parser = new nokogiri($html);
			    $data = reset($parser->get($instruction)->toArray());
			    //print_r($data);exit;
			  	if (isset($data['#text'])){
		       ...
		       if(count($res_t) > 1){
		       		...
		       		if(count($res_t) > 1){
		       			$res_t = explode('</table>' , $res_t[1] , 2);
		       			if(count($res_t) > 1){
		       				$res .= '<table>' . str_replace(array("&nbsp;" , "&amp;") , array(" " , "`") , enc($res_t[0]) ) . '</table>';
		       			}	
		       		}
...
function get_main_image($html){
        ...
        if ($data) {
            $data = reset($data);
            return prepare_big_image($data['src']);
        }
        ...
        if(count($data) > 0 && isset($data[0]) && isset($data[0]['src'])){
        	return prepare_big_image($data[0]['src']);
...
function get_other_images($html){
    ...
    function prepare_big_image($src){
    	return str_replace(array("_250x250.jpg") , array("")  , $src);
    }
