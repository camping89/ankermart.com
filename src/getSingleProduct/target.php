...
function get_title($html){
		$instruction = 'h2.product-name span';
...
function get_spec($html){
		...
		$temp  = $pq->find('ul.innerlistings');
		foreach ($temp as $block){
				$res .= '<br /><br />'.pq($block)->html();
				break;
		}
		
		
		// ProductSpecification подтягивается аяксом
		$spec_res = explode('"catalogEntryID":"' , $html);
		if(count($spec_res) > 1){
			$spec_res = explode('"' , $spec_res[1] , 2);
			if(count($spec_res) > 1){
				$spec_res = (int) $spec_res[0];
				$ch = curl_init ("http://www.target.com/ProductSpecificationView?tabName=item-specifications");  
				curl_setopt($ch,CURLOPT_FOLLOWLOCATION,true);
				curl_setopt ($ch, CURLOPT_RETURNTRANSFER, true);
				curl_setopt($ch, CURLOPT_POST, true);
    			curl_setopt($ch, CURLOPT_POSTFIELDS, "sSoftRefresh=true&catEntryId=$spec_res&parentId=$spec_res&enablePluckFeature=Y");
				$output = curl_exec ($ch);
				$res .= '<br />'.$output;
				//isSoftRefresh:true
				//catEntryId:204545088
				//parentId:204545088
				//enablePluckFeature:Y
...
function get_other_images($html){
        ...
        $instruction = 'div.carouselviewport ul li a img';
        $parser = new nokogiri($html);
        $data = $parser->get($instruction)->toArray();
   		if(count($data) > 1 && isset($data[0]) ){
   			...
   			foreach($data as $img){
   				if(isset($img['src'])){
   					$result[] = prepare_big_image($img['src']);
   				}	
   			}
        }
    ...
    function prepare_big_image($src){
    	return str_replace(array("_50x50") , array("")  , $src);
    }
