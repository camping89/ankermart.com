...
// TODO:
/*
 * получить источники изображений в media галерее можно по ссылке
 *  http://ws.cnetcontent.com/269d7425/script/fd7be35fd9?cpn=757146&mf=Dell&pn=I15RV-1434BLK&upcean=884116132431&ccid=products&lang=en&market=US&host=www.microcenter.com
 *  
 *  ссылка формируется из данных которые можно получить на исходной странице
 * 
 */
...
if(!empty($_SERVER['HTTP_X_REQUESTED_WITH']) 
	...
	// check url
	if(strpos($data['url'] , "microcenter")){
		...
		/********************/
		if($data["spec"] == "on"){
			$out["spec"] = get_spec($output , $data["spec_over"] , $data["spec_spec"] , $data["spec_more"] , $data["spec_warr"]);
...
function get_spec($html ,  $spec_over , $spec_spec , $spec_more , $spec_warr){
		$out = '';
		
		//SPECS TAB
		if($spec_over == "on"){
			$pq = phpQuery::newDocumentHTML($html);
			$temp  = $pq->find('article#tab-overview');
			foreach ($temp as $block){
				$out .= clear_overview_tab($temp->html()).'<br />';
			}
		}
		...
		if($spec_spec == "on"){
			$pq = phpQuery::newDocumentHTML($html);
			$temp  = $pq->find('article#tab-specs');
			foreach ($temp as $block){
				$out .= $temp->html().'<br />';
			}
		}
		...
		if($spec_more == "on"){
			$pq = phpQuery::newDocumentHTML($html);
			$temp  = $pq->find('article#tab-info');
			foreach ($temp as $block){
				$out .= $temp->html().'<br />';
			}
		}
		
		// WARRANTY AND SUPPORT TAB
		if($spec_warr == "on"){
			$pq = phpQuery::newDocumentHTML($html);
			$temp  = $pq->find('article#tab-support');
			foreach ($temp as $block){
				$out .= $temp->html();
			}
		}
		...
		//echo $out;exit;
        return clear_scripts($out);
}


function clear_overview_tab($res){
	$out = '';
	
	if(strpos($res , '<div class="fb-container"') > 10 && strpos($res , '<div id="flix-minisite"></div>') > 10){
		$res_t = explode('<div class="fb-container"' , $res , 2);
		$out .= $res_t[0];
		$res_t = explode('<div id="flix-minisite"></div>' , $res_t[1] , 2);
		$out .= $res_t[1];
	}else{
		return $res;
	}
...
function get_images_arr($html){
	...
	$instruction = 'div.slides-container a img';
    $parser = new nokogiri($html);
    $data = $parser->get($instruction)->toArray();
    //echo '<pre>'.print_r($data , 1).'</pre>';exit;
    if(count($data) > 0 && is_array($data)){
    	foreach($data as $img){
    		if(isset($img['src'])){
    			$out[] = str_ireplace(array("thumbnail") , array("comping") , $img['src']);
    		}
    	}
    }
