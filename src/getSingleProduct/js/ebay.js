...
if(!isset("GetSingleProduct_assembly") || !isset("GetSingleProduct_assembly.rootLocation") || !isset("GetSingleProduct_assembly.adminLocation") ){
	...
	GetSingleProduct_ebay.rootLocation = GetSingleProduct_assembly.rootLocation;
	GetSingleProduct_ebay.adminLocation = GetSingleProduct_assembly.adminLocation;
}
GetSingleProduct_ebay.imageFolderName = "";
...
GetSingleProduct_ebay.LANG_IDS = new Array();
...
GetSingleProduct_ebay.initialForm_ebay += '<br /><input type="checkbox" id="ebay_import_allLanguages" checked="checked"/>&nbsp;&nbsp;&nbsp;'
	GetSingleProduct_ebay.initialForm_ebay += '<label for="ebay_import_allLanguages"><b>Insert grabbed data in ALL language tabs</b></label><br /><br />';
...
$(document).ready(function(){
	...
	$("#" + GetSingleProduct_ebay.tabGeneral + " span.required").parent().append('<br />&nbsp;<a onclick="parse_ebay($(this));" >Get product from ' + GetSingleProduct_ebay.marketName + '</a>');
...
function parse_ebay(obj){
	var lang_id = get_current_lang_ebay(obj);
	...
	if(GetSingleProduct_ebay.ocVersion > 4){
		$( "#dialog_ebay_import" ).dialog({ width: 530 ,
												buttons: [{
		                                                  	...
		                                                  	click: function() { get_form_data_ebay(lang_id); }
		                                                  },{
		                                                	text: "Cancel",
		...
		$( "#dialog_ebay_import" ).dialog({ width: 530 ,
											buttons: {
		                                    	"Get this product": function() { get_form_data_ebay(lang_id); },
...
function get_current_lang_ebay(obj){
	var attrName = obj.parent().parent().parent().find("td input:first").attr("name");
	var res = SPS_ebay_parse_explode("description[" , attrName);
	if(res.length > 1){
...
function get_all_langsIDS_ebay(){
	var RES_LANG_IDS = [];
...
function get_form_data_ebay(lang_id){
	if(GetSingleProduct_ebay.semafor < 1){
		...
		var import_allLanguages_ebay = $("#ebay_import_allLanguages").attr("checked") == "checked"?1:0;
		GetSingleProduct_ebay.LANG_IDS = [];
		if(import_allLanguages_ebay > 0){
			GetSingleProduct_ebay.LANG_IDS = get_all_langsIDS_ebay();
		}else{
			GetSingleProduct_ebay.LANG_IDS.push(lang_id);
		}
							...
							$.each( GetSingleProduct_ebay.fields, function( key, value ) {
								...
								if(datakey !== undefined && windowkey == "on"){
									switch(key){
										// TITLE
										case 'title':
											$.each( GetSingleProduct_ebay.LANG_IDS, function( key, val ) {
												$("input[name='product_description["+val+"][name]']").val(oData.title);
											});
											$("input[name='keyword']").val( SPS_ebay_prepare_seokeys(oData.title) );
											break;
										// META DESCRIPTION
										case 'desc':
											$.each( GetSingleProduct_ebay.LANG_IDS, function( key, val ) {
												...
												if(import_allLanguages_ebay > 0){
													...
													try {
														...
														$("iframe").each(function(){ 
															...
															var ifrID = SPS_ebay_parse_explode("description" , ifrTITLE);
															if(ifrID.length > 1){
...
function get_folder_name_ebay(name){
	var out = name;
	var res = SPS_ebay_parse_explode(" " , name);
	if(res.length > 5){
...
function SPS_ebay_prepare_seokeys(name) {
	var res = SPS_ebay_parse_str_replace(" " , "-" , name);
