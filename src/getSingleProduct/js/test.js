$(document).ready(function(){
	$("#tab-general span.required").parent().append('<br />&nbsp;<a onclick="parse_ali($(this).parent().parent().parent().parent().parent().attr(\'id\'));" >Get product from Aliexpress.com</a>');
	
	$("#tab-data").append('<div id="dialog_ali_import" title="Copy product URL from Aliexpress.com">'+
							'</div>');
...
function parse_ali(id){
	$( "#dialog_ali_import" ).html(GetSingleProduct.initialForm);
	$( "#dialog_ali_import" ).dialog({ width: 530 , buttons: [
	                                                           {
	                                                               text: "Get this product",
	                                                               click: function() { get_form_data_ali(id); }
	                                                           },{
	                                                        	   text: "Cancel",
...
function get_form_data_ali(id){
	...
	if(GetSingleProduct.semafor < 1){
		...
		var product_name = parseName_ali(url);
		if(product_name !== false){
			...
			var folder = 'from Aliexpress - ' + get_folder_name_ali(product_name)+ ' ' + Math.floor(Math.random()* 100);;
			...
			$.post(GetSingleProduct.rootLocation+"/admin/index.php?route=common/filemanager/create&token="+GetSingleProduct.token , {"directory" : "" , "name" : folder} , function(data){
				...
				if(oData.success !== undefined){
					$.post(GetSingleProduct.rootLocation+"/getSingleProduct/ali.php" , {"data[url]" : url , "data[images]" : images , "data[title]" : title , "data[desc]" :  desc , "data[spec]" : spec , "data[keys]" : keys , "token" : GetSingleProduct.token , "tempdir" : folder } , function(data){
						$( "#dialog_ali_import" ).dialog('close');
						//eval("var oData = " + data + ";");
						var oData = data;
						//console.log(oData);
...
function parseName_ali(url){
	var res = parse_explode("aliexpress" , url);
	if(res.length > 1){
...
function get_folder_name_ali(name){
