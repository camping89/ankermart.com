...
if(!isset("GetSingleProduct_assembly") || !isset("GetSingleProduct_assembly.rootLocation") || !isset("GetSingleProduct_assembly.adminLocation") ){
	GetSingleProduct_aliexpress.rootLocation = "";
	GetSingleProduct_aliexpress.adminLocation = "admin";
}else{
	GetSingleProduct_aliexpress.rootLocation = GetSingleProduct_assembly.rootLocation;
	GetSingleProduct_aliexpress.adminLocation = GetSingleProduct_assembly.adminLocation;
}
...
GetSingleProduct_aliexpress.ParsingScriptName = "aliexpress";
...
GetSingleProduct_aliexpress.initialForm_aliexpress = '<div><p id="SPS_dialog_body_aliexpress">URL: <input type="text" id="aliexpress_import_parse_url" style="width:400px;" /><br /><br /><br />';
$.each( GetSingleProduct_aliexpress.fields, function( key, value ) {
	GetSingleProduct_aliexpress.initialForm_aliexpress += '<input type="checkbox" id="aliexpress_import_parse_' + key + '" checked="checked"/>&nbsp;&nbsp;&nbsp;';
	GetSingleProduct_aliexpress.initialForm_aliexpress += '<label for="aliexpress_import_parse_' + key + '">' + value + '</label><br /><br />';
	});
GetSingleProduct_aliexpress.initialForm_aliexpress += '<br /><input type="checkbox" id="aliexpress_import_allLanguages" checked="checked"/>&nbsp;&nbsp;&nbsp;'
	GetSingleProduct_aliexpress.initialForm_aliexpress += '<label for="aliexpress_import_allLanguages"><b>Insert grabbed data in ALL language tabs</b></label><br /><br />';
...
$(document).ready(function(){
	...
	if($("#" + GetSingleProduct_aliexpress.tabGeneral).html() == null){
		GetSingleProduct_aliexpress.tabGeneral = "tab_general";
	}
	GetSingleProduct_aliexpress.tabData = "tab-data";
	if($("#" + GetSingleProduct_aliexpress.tabData).html() == null){
		GetSingleProduct_aliexpress.tabData = "tab_data";
	}
	$("#" + GetSingleProduct_aliexpress.tabGeneral + " span.required").parent().append('<br />&nbsp;<a onclick="parse_aliexpress($(this));" >Get product from ' + GetSingleProduct_aliexpress.marketName + '</a>');
	
	$("#" + GetSingleProduct_aliexpress.tabData).append('<div id="dialog_aliexpress_import" title="Copy product URL from ' + GetSingleProduct_aliexpress.marketName + '">'+
							'</div>');
	...
	if(ver > 1){
		GetSingleProduct_aliexpress.ocVersion = 4;
	}
...
function parse_aliexpress(obj){
	...
	$( "#dialog_aliexpress_import" ).html(GetSingleProduct_aliexpress.initialForm_aliexpress);
	if(GetSingleProduct_aliexpress.ocVersion > 4){
		$( "#dialog_aliexpress_import" ).dialog({ width: 530 ,
												buttons: [{
		                                                  	text: "Get this product",
		                                                  	click: function() { get_form_data_aliexpress(lang_id); }
		                                                  },{
		                                                	text: "Cancel",
		                                                    click: function() { if(GetSingleProduct_aliexpress.semafor < 1){ $(this).dialog("close");} }
		                                       }]});
		...
		$( "#dialog_aliexpress_import" ).dialog({ width: 530 ,
											buttons: {
		                                        ...
		                                        "Cancel" : function() { if(GetSingleProduct_aliexpress.semafor < 1){ $(this).dialog("close");} }
		                                    }
										});
	}
}
...
function get_form_data_aliexpress(lang_id){
	if(GetSingleProduct_aliexpress.semafor < 1){
		...
		var url = $("#aliexpress_import_parse_url").val();
		//console.log("url:" + url);
		if(GetSingleProduct_aliexpress.ocVersion > 4){
			$.each( GetSingleProduct_aliexpress.fields, function( key, value ) {
				eval('window.' + key + ' = $("#aliexpress_import_parse_' + key + '").attr("checked") == "checked"?"on":"off";');
			});
		}else{
			$.each( GetSingleProduct_aliexpress.fields, function( key, value ) {
				eval('window.' + key + ' = $("#aliexpress_import_parse_' + key + '").attr("checked") == true?"on":"off";');
			});
		}
		
		// получаем настройку с формы
		var import_allLanguages_aliexpress = $("#aliexpress_import_allLanguages").attr("checked") == "checked"?1:0;
		...
		var product_name = parseName_aliexpress(url);
		if(product_name !== false){
			GetSingleProduct_aliexpress.tempStore = $("#SPS_dialog_body_aliexpress").html();
			GetSingleProduct_aliexpress.semafor = 1;
			$("#SPS_dialog_body_aliexpress").html('<div style="width:100%;text-align:center;">Please, wait. This may take several minutes...</div>');
			
			// create  product images folder from here
			var folder = GetSingleProduct_aliexpress.imageFolderName + get_folder_name_aliexpress(product_name) + ' ' + Math.floor(Math.random()* 100000);
			var folderPath = GetSingleProduct_aliexpress.rootLocation+"/" + GetSingleProduct_aliexpress.adminLocation + "/index.php?route=common/filemanager/create&token="+GetSingleProduct_aliexpress.token;
			...
			$.post(folderPath , folderData , function(data){
				...
				if(oData.success !== undefined || error_on_creating_folder > 0 ){
					var parsingPath = GetSingleProduct_aliexpress.rootLocation + "/getSingleProduct/" + GetSingleProduct_aliexpress.ParsingScriptName + ".php";
					var parsingData = {
					...
					$.post(parsingPath , parsingData , function(data){
						...
						if(oData.title == undefined && oData.spec == undefined && oData.main_image == undefined && oData.desc == undefined && oData.price == undefined && oData.keywords == undefined && oData.model == undefined && oData.sku == undefined){
								...
								$("#SPS_dialog_body_aliexpress").html(GetSingleProduct_aliexpress.tempStore);
						}else{
							...
							$.each( GetSingleProduct_aliexpress.fields, function( key, value ) {
								...
								if(datakey !== undefined && windowkey == "on"){
									switch(key){
										// TITLE
										case 'title':
											$.each( GetSingleProduct_aliexpress.LANG_IDS, function( key, val ) {
												$("input[name='product_description["+val+"][name]']").val(oData.title);
											});
											$("input[name='keyword']").val( SPS_aliexpress_prepare_seokeys(oData.title) );
											break;
										// META DESCRIPTION
										case 'desc':
											$.each( GetSingleProduct_aliexpress.LANG_IDS, function( key, val ) {
							...
							if(main_image !== undefined){
								if(main_image.length > 0){
									var main_path = GetSingleProduct_aliexpress.rootLocation+"/image/data/" + folder + "/main." + oData.main_image;
							...
							if(other_images !== undefined && images == "on"){
								if(other_images.length > 0){
									...
									for(var oth = last_image_num; oth < (other_images.length + last_image_num); oth++){
										addImage();
										var other_path = GetSingleProduct_aliexpress.rootLocation+"/image/data/" + folder + "/" + parseInt(oth - last_image_num  + 1) + "." + oData.other_images[oth - last_image_num];
							...
							$( "#dialog_aliexpress_import" ).dialog('close');
 ...
 */
function parseName_aliexpress(url){
	// 2 variations
	// http://www.aliexpressexpress.com/store/product/1pcs-Free-shipping-Survivor-luxury-water-dirt-shock-proof-senior-military-duty-support-case-for-Samsung/818283_750977644.html
	// http://www.aliexpressexpress.com/item/Wholesale-NWT-4-pcs-POCOYO-FRIENDS-BANDAI-PLUSH-Pocoyo-Elly-Pato-Loula-4-Dolls-FREE-SHIPPING/649481932.html
	
	var res = SPS_aliexpress_parse_explode("aliexpress" , url);
	if(res.length > 1){
		res = SPS_aliexpress_parse_explode("/" , res[1]);
		if(res.length > 2){
			// first var
			...
			res = SPS_aliexpress_parse_str_replace("-" , " " , res);
			if(res.length > 5){
				return res;
			}else{
				return 'product ';
