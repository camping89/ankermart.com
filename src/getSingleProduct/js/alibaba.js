...
if(!isset("GetSingleProduct_assembly") || !isset("GetSingleProduct_assembly.rootLocation") || !isset("GetSingleProduct_assembly.adminLocation") ){
	GetSingleProduct_alibaba.rootLocation = "";
	GetSingleProduct_alibaba.adminLocation = "admin";
}else{
	GetSingleProduct_alibaba.rootLocation = GetSingleProduct_assembly.rootLocation;
	GetSingleProduct_alibaba.adminLocation = GetSingleProduct_assembly.adminLocation;
}
...
GetSingleProduct_alibaba.ParsingScriptName = "alibaba";
...
GetSingleProduct_alibaba.initialForm_alibaba = '<div><p id="SPS_dialog_body_alibaba">URL: <input type="text" id="alibaba_import_parse_url" style="width:400px;" /><br /><br /><br />';
$.each( GetSingleProduct_alibaba.fields, function( key, value ) {
	GetSingleProduct_alibaba.initialForm_alibaba += '<input type="checkbox" id="alibaba_import_parse_' + key + '" checked="checked"/>&nbsp;&nbsp;&nbsp;';
	GetSingleProduct_alibaba.initialForm_alibaba += '<label for="alibaba_import_parse_' + key + '">' + value + '</label><br /><br />';
	});
GetSingleProduct_alibaba.initialForm_alibaba += '<br /><input type="checkbox" id="alibaba_import_allLanguages" checked="checked"/>&nbsp;&nbsp;&nbsp;'
	GetSingleProduct_alibaba.initialForm_alibaba += '<label for="alibaba_import_allLanguages"><b>Insert grabbed data in ALL language tabs</b></label><br /><br />';
...
$(document).ready(function(){
	...
	if($("#" + GetSingleProduct_alibaba.tabGeneral).html() == null){
		GetSingleProduct_alibaba.tabGeneral = "tab_general";
	}
	GetSingleProduct_alibaba.tabData = "tab-data";
	if($("#" + GetSingleProduct_alibaba.tabData).html() == null){
		GetSingleProduct_alibaba.tabData = "tab_data";
	}
	$("#" + GetSingleProduct_alibaba.tabGeneral + " span.required").parent().append('<br />&nbsp;<a onclick="parse_alibaba($(this));" >Get product from ' + GetSingleProduct_alibaba.marketName + '</a>');
	
	$("#" + GetSingleProduct_alibaba.tabData).append('<div id="dialog_alibaba_import" title="Copy product URL from ' + GetSingleProduct_alibaba.marketName + '">'+
							'</div>');
	...
	if(ver > 1){
		GetSingleProduct_alibaba.ocVersion = 4;
	}
...
function parse_alibaba(obj){
	...
	$( "#dialog_alibaba_import" ).html(GetSingleProduct_alibaba.initialForm_alibaba);
	if(GetSingleProduct_alibaba.ocVersion > 4){
		$( "#dialog_alibaba_import" ).dialog({ width: 530 ,
												buttons: [{
		                                                  	text: "Get this product",
		                                                  	click: function() { get_form_data_alibaba(lang_id); }
		                                                  },{
		                                                	text: "Cancel",
		                                                    click: function() { if(GetSingleProduct_alibaba.semafor < 1){ $(this).dialog("close");} }
		                                       }]});
		...
		$( "#dialog_alibaba_import" ).dialog({ width: 530 ,
											buttons: {
		                                        ...
		                                        "Cancel" : function() { if(GetSingleProduct_alibaba.semafor < 1){ $(this).dialog("close");} }
		                                    }
										});
	}
}
...
function get_form_data_alibaba(lang_id){
	if(GetSingleProduct_alibaba.semafor < 1){
		...
		var url = $("#alibaba_import_parse_url").val();
		//console.log("url:" + url);
		if(GetSingleProduct_alibaba.ocVersion > 4){
			$.each( GetSingleProduct_alibaba.fields, function( key, value ) {
				eval('window.' + key + ' = $("#alibaba_import_parse_' + key + '").attr("checked") == "checked"?"on":"off";');
			});
		}else{
			$.each( GetSingleProduct_alibaba.fields, function( key, value ) {
				eval('window.' + key + ' = $("#alibaba_import_parse_' + key + '").attr("checked") == true?"on":"off";');
			});
		}
		
		// получаем настройку с формы
		var import_allLanguages_alibaba = $("#alibaba_import_allLanguages").attr("checked") == "checked"?1:0;
		...
		var product_name = parseName_alibaba(url);
		if(product_name !== false){
			GetSingleProduct_alibaba.tempStore = $("#SPS_dialog_body_alibaba").html();
			GetSingleProduct_alibaba.semafor = 1;
			$("#SPS_dialog_body_alibaba").html('<div style="width:100%;text-align:center;">Please, wait. This may take several minutes...</div>');
			
			// create  product images folder from here
			var folder = GetSingleProduct_alibaba.imageFolderName + get_folder_name_alibaba(product_name) + ' ' + Math.floor(Math.random()* 100000);
			var folderPath = GetSingleProduct_alibaba.rootLocation+"/" + GetSingleProduct_alibaba.adminLocation + "/index.php?route=common/filemanager/create&token="+GetSingleProduct_alibaba.token;
			...
			$.post(folderPath , folderData , function(data){
				...
				if(oData.success !== undefined || error_on_creating_folder > 0 ){
					var parsingPath = GetSingleProduct_alibaba.rootLocation + "/getSingleProduct/" + GetSingleProduct_alibaba.ParsingScriptName + ".php";
					var parsingData = {
					...
					$.post(parsingPath , parsingData , function(data){
						...
						if(oData.title == undefined && oData.spec == undefined && oData.main_image == undefined && oData.desc == undefined && oData.price == undefined && oData.keywords == undefined && oData.model == undefined && oData.sku == undefined){
								...
								$("#SPS_dialog_body_alibaba").html(GetSingleProduct_alibaba.tempStore);
						}else{
							...
							$.each( GetSingleProduct_alibaba.fields, function( key, value ) {
								...
								if(datakey !== undefined && windowkey == "on"){
									switch(key){
										// TITLE
										case 'title':
											$.each( GetSingleProduct_alibaba.LANG_IDS, function( key, val ) {
												$("input[name='product_description["+val+"][name]']").val(oData.title);
											});
											$("input[name='keyword']").val( SPS_alibaba_prepare_seokeys(oData.title) );
											break;
										// META DESCRIPTION
										case 'desc':
											$.each( GetSingleProduct_alibaba.LANG_IDS, function( key, val ) {
							...
							if(main_image !== undefined){
								if(main_image.length > 0){
									var main_path = GetSingleProduct_alibaba.rootLocation+"/image/data/" + folder + "/main." + oData.main_image;
							...
							if(other_images !== undefined && images == "on"){
								if(other_images.length > 0){
									...
									for(var oth = last_image_num; oth < (other_images.length + last_image_num); oth++){
										addImage();
										var other_path = GetSingleProduct_alibaba.rootLocation+"/image/data/" + folder + "/" + parseInt(oth - last_image_num  + 1) + "." + oData.other_images[oth - last_image_num];
							...
							$( "#dialog_alibaba_import" ).dialog('close');
 ...
 */
function parseName_alibaba(url){
	var res = SPS_alibaba_parse_explode("alibaba" , url);
	if(res.length > 1){
		res = SPS_alibaba_parse_explode("/" , res[1]);
		if(res.length > 2){
			res = res[2];
			res = SPS_alibaba_parse_str_replace("-" , " " , res);
			if(res.length > 5){
				return res;
			}else{
				return 'product ';
		...
		var res = SPS_alibaba_parse_explode("1688.com/offer/" , url);
		if(res.length > 1){
			res = SPS_alibaba_parse_str_replace(".html" , " " , res[1]);
			if(res.length > 2){
				return res;
			}else{
				return 'product ';
