$(document).ready(function(){
	$("#tab-general span.required").parent().append('<br />&nbsp;<a onclick="parse_hbw($(this).parent().parent().parent().parent().parent().attr(\'id\'));" >Get product from HBW.my</a>');
	
	$("#tab-data").append('<div id="dialog_hbw_import" title="Copy product URL from HandBagWholesale.my">'+
							'</div>');
...
function parse_hbw(id){
	$( "#dialog_hbw_import" ).html(GetSingleProduct.initialForm);
	$( "#dialog_hbw_import" ).dialog({ width: 530 , buttons: [
	                                                           {
	                                                               text: "Get this product",
	                                                               click: function() { get_form_data_hbw(id); }
	                                                           },{
	                                                        	   text: "Cancel",
...
function get_form_data_hbw(id){
	...
	if(GetSingleProduct.semafor < 1){
		...
		var product_name = parseName_hbw(url);
		if(product_name !== false){
			...
			var folder = 'from HandBagWholesale - ' + get_folder_name_hbw(product_name)+ ' ' + Math.floor(Math.random()* 10000);;
			...
			$.post(GetSingleProduct.rootLocation+"/admin/index.php?route=common/filemanager/create&token="+GetSingleProduct.token , {"directory" : "" , "name" : folder} , function(data){
				...
				if(oData.success !== undefined){
					$.post(GetSingleProduct.rootLocation+"/getSingleProduct/hbw.php" , {"data[url]" : url , "data[images]" : images , "data[title]" : title , "data[desc]" :  desc , "data[spec]" : spec , "data[keys]" : keys , "token" : GetSingleProduct.token , "tempdir" : folder } , function(data){
						$( "#dialog_hbw_import" ).dialog('close');
						//eval("var oData = " + data + ";");
						var oData = data;
						//console.log(oData);
...
function parseName_hbw(url){
	var res = parse_explode("handbagwholesale.my" , url);
	if(res.length > 1){
...
function get_folder_name_hbw(name){
