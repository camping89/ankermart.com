...
function get_price($html){
		$instruction = 'span#CenterContentPlaceHolderM__priceLabelIC';
	    $parser = new nokogiri($html);
	    $data = $parser->get($instruction)->toArray();
	    //echo '<pre>'.print_r($data , 1).'</pre>';exit;	
	    unset($parser);
	    if (isset($data[0]['#text']) && !is_array($data[0]['#text'])) {
	    	//echo trim(str_ireplace(array("," , "€", "$" , "USD") , array("." , "" , "" , "" ) , $data[0]['#text']));
	    	return (float) trim(str_ireplace(array("," , "€", "$" , "USD") , array("." , "" , "" , "" ) , $data[0]['#text']));
        }
...
function get_spec($html){
		...
		$temp  = $pq->find('div#tabs-1');
		foreach ($temp as $block){
			$res .= $temp->html().'<br />';
		}
		
		$temp  = $pq->find('div#tabs-2');
		foreach ($temp as $block){
			$res .= $temp->html().'<br />';
		}
...
function get_main_image($html){
		...
		$instruction = 'img#CenterContentPlaceHolderM__productImage';
	    $parser = new nokogiri($html);
		...
		if (isset($res[0]['src'])) {
	    	...
	    	return 'http://www.e-key.it/'.$main_image;
        }
